...
# new function to handle taxa summaries generated by mctoolsr. will depricate old function in future
taxa_summary_by_sample_type = function(taxa_smry_df, metadata_map, factor, filter_level, test_type, grouping_factor, out_fp){
  if(!missing(filter_level)){
    # filter taxa summary table by abundance in any/either factor level
    taxa_smry_df = filter_taxa_dit(taxa_smry_df, metadata_map, filter_level, factor)
  }
  if(!missing(grouping_factor)){
    test_results = run_test(taxa_smry_df, metadata_map, factor, test_type, grouping_factor)
  }
  ...
  # output data
  if(!missing(out_fp)){
    write.table(test_results, file = out_fp, sep = ",", row.names = TRUE, col.names = NA)  
  } else test_results
}
